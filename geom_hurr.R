...
geom_hurricane_proto <- ggplot2::ggproto(
    ...
    draw_group = function(data, panel_scales, coord) {
        ...
        data <- data %>% 
            mutate_(r_ne = ~ r_ne * 1852 * scale_radii,
        ...
        for (i in 1:nrow(data)) {
            ...
            df_se <- base::data.frame(
                colour = data[i,]$colour,fill = data[i,]$fill,
                geosphere::destPoint(p = c(data[i,]$x, data[i,]$y),
                b = 90:180,
                d = data[i,]$r_se),
                group = data[i,]$group,
...
ike_data <- load_hdata('data/ebtrk_atlc_1988_2015.txt') %>% 
    tidy_hdata() %>% 
    filter_hdata(hurricane = 'Ike-2008', 
                 observation = '2008-09-13 12:00:00')



# Create map and add hurricane Ike storm observation
map_ike <- get_stamenmap(bbox = c(-94.0434, 28.9287, -88.8165, 33.0197),
...
louisiana_bbox <- c(-94.0434, 28.9287, -88.8165, 33.0197)
texas_bbox <- c(-106.6460, 25.8371, -93.5083, 36.5007)
...
geom_hurricane(data = ike_data,
               aes(x = longitude, y = latitude, 
                   r_ne = ne, r_se = se, r_nw = nw, r_sw = sw,
                   fill = wind_speed, color = wind_speed), alpha = 0.7) + 
    scale_color_manual(name = "Wind speed (kts)", 
                       values = c("red", "orange", "yellow")) + 
    scale_fill_manual(name = "Wind speed (kts)", 
                      values = c("red", "orange", "yellow")) +
    ggtitle("Wind speeds for hurricane Ike on September 13, 2008 at 12:00:00")
